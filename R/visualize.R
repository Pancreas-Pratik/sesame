...
visualizeRegion <- function(chrm, beg, end, betas, platform = NULL,
    ...
    na.rm = FALSE, nprobes.max = 1000, txn.types = NULL,
    font.size.scaling.factor = 0.5, ...) {
  
  font.size.scaling.factor = font.size.scaling.factor
  txn.font.size = as.numeric(8*font.size.scaling.factor)
    ...
    plt.cytoband <- plotCytoBand(chrm, beg, end, genomeInfo, txn.font.size)
    plt.loci_txn_intersectionLines <- plotLociTxnIntersectionLines(probes, beg, end, txns)
